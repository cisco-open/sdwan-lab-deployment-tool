...
def main() -> None:
    ...
    task_subparsers = main_parser.add_subparsers(
        help="Task to be performed.", dest="task"
    )
    task_subparsers.required = True

    setup_parser = task_subparsers.add_parser(
    ...
    deploy_parser = task_subparsers.add_parser(
        "deploy", help="Deploy a new Catalyst SD-WAN lab pod."
    )
    ...
    add_parser = task_subparsers.add_parser(
        "add", help="Add Catalyst SD-WAN device to running lab pod."
    )
    ...
    backup_parser = task_subparsers.add_parser(
        "backup", help="Backup running Catalyst SD-WAN lab pod."
    )
    ...
    restore_parser = task_subparsers.add_parser(
        "restore", help="Restore Catalyst SD-WAN POD from backup."
    )
    ...
    delete_parser = task_subparsers.add_parser(
        "delete", help="Delete the CML lab and all the lab data."
    )
    ...
    cml = ClientLibrary(
        "https://" + cli_args.cml, cli_args.user, cli_args.password, ssl_verify=False
    )
    ...
    if cli_args.task == "setup":
        ...
        add.main(
            ...
            cli_args.mpassword,
            cli_args.lab,
            cli_args.number_of_devices,
            cli_args.device_type.lower(),
            cli_args.software_version,
            cli_args.loglevel,
        )
