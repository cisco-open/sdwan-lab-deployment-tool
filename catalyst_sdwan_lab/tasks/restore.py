...
def main(cml, cml_ip, manager_ip, manager_mask, manager_gateway, manager_user, manager_password,
    ...
    if missing_images:
        sys.exit(f'The following image definitions are required, but not present: {missing_images}\n'
    ...
    if deleteexisitng:
        ...
        lab = next((lab for lab in cml.find_labs_by_title(cml_topology_dict['lab']['title'])
                    if manager_ip in lab.notes), None)
        if lab:
    ...
    manager_session = wait_for_manager_session(manager_ip, manager_user, manager_password, log)
        ...
        with Rest(base_url=f'https://{manager_ip}', username=manager_user, password=manager_password) as api:
            ...
            if not mrf_regions_configured:
                if major_software_release == 20 and minor_software_release in [7, 8, 9, 10, 11, 12]:
                    ...
                    api.post({'data': {'enableMrfInterRegionRouting': {'optionType': 'global', 'value': True}}},
                             f'v1/network-hierarchy/{global_id}/network-settings/mrf')
                # Check network hierarchy global ID
                # noinspection PyTypeChecker
                network_hierarchy: List = api.get('v1/network-hierarchy')
                global_id = (next(_['id'] for _ in network_hierarchy if _['data']['label'] == 'GLOBAL'))
                            ...
                            if filename.endswith('.json'):
                                with open(f'{path}/{filename}', 'r') as file:
                                    ...
                                    if region_dict['name'] not in existing_region_names:
                                        log.info(f'Creating MRF region {region_dict["name"]}...')
                                        old_id = region_dict['uuid']
                                        region_dict.pop('uuid', None)
                                        if folder == 'regions':
                                            region_dict['data']['parentUuid'] = global_id
    ...
    controllers_uuids = {device_ip_to_system_ip[device.device_ip]: device.uuid
        ...
        for filename in os.listdir(join(manager_configs_dir, 'device_templates', 'attached')):
            with open(join(manager_configs_dir, 'device_templates', 'attached', filename), 'r+') as f1:
                ...
                if data[0]['personality'] == 'vsmart':
                    ...
                    with open(join(manager_configs_dir, 'device_templates', 'values', filename), 'r+') as f2:
                        data = json.load(f2)
                        for device in data['data']:
                            device['csv-deviceId'] = controllers_uuids[device['csv-deviceIP']]
                        f2.seek(0)
                        json.dump(data, f2, indent=2)

    restore_manager_configuration(manager_ip, manager_user, manager_password, join(workdir, manager_configs_dir), True)

    uuid_to_token = {device.uuid: device.serial_number for device in device_inventory.get_device_details('vedges')}
    ...
    for node in lab.nodes():
        if node.node_definition == 'cat-sdwan-edge' and node.is_booted() is False:
            ...
            node.config = re.sub(r'(vinitparam:[\w\W]+?otp\s:)\s(\w+)', rf'\1 {token}', node.config)
            node.start()
