...
def main(cml, cml_ip, manager_ip, manager_mask, manager_gateway, manager_user, manager_password,
         software_version, lab_name, bridge, dns_server, retry, loglevel):
    ...
    manager_image = get_cml_sdwan_image_definition(cml, 'cat-sdwan-manager', software_version)
    controller_image = get_cml_sdwan_image_definition(cml, 'cat-sdwan-controller', software_version)
    validator_image = get_cml_sdwan_image_definition(cml, 'cat-sdwan-validator', software_version)
    ...
    if not lab_name:
        ...
        lab_list = [int(re.search(r'sdwan(\d+)', lab.title).group(1))
    ...
    cml_topology = cml_tp_tmpl.render(title=lab_name, manager_image=manager_image,
                                      ...
                                      external_gateway=manager_gateway, bridge=bridge, dns_server=dns_server)
