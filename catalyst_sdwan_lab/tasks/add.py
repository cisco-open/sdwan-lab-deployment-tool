...
def ping_node(vpn_ip, manager_session, manager_system_ip):
    ...
    nping = manager_session.endpoints.troubleshooting_tools.device_connectivity.nping_device
    retries = 0
    max_retries = 30
...
def main(cml, cml_user, cml_password, manager_ip, manager_user, manager_password,
         lab_name, number_of_devices, device_type, software_version, loglevel):
    ...
    if device_type in ['vbond', 'validator']:
        ...
        exit(f'Adding {device_type} is not supported by the script. '
    ...
    image_definition = get_cml_sdwan_image_definition(cml, cml_node_type, software_version)
    ...
    if cml_node_type in ['cat-sdwan-validator', 'cat-sdwan-controller']:
        ...
        if major_software_release < 17 or (major_software_release == 17 and minor_software_release < 12):
    ...
    if lab:
        ...
        if cml_node_type in ['cat-sdwan-validator', 'cat-sdwan-controller']:
            ...
            for next_num_str in new_nodes_nums:
                i += 1
                track_progress(log, f'Adding nodes to topology ({i}/{len(new_nodes_nums)})...')
                ...
                node = lab.create_node(label=label, node_definition=cml_node_type,
                                       image_definition=image_definition,
                                       configuration=bootstrap_config, populate_interfaces=True,
                                       x=next_node_x_position, y=next_node_y_position)
            ...
            vpn0_ip_prefix = {'vmanage': '172.16.0.',
                              'vsmart': '172.16.0.1',
                              'vbond': '172.16.0.2'}
            ...
            if cml_node_type == 'cat-sdwan-validator':
                ...
                track_progress(log, 'Updating SD-WAN Validator FQDN entry on Gateway...')
                ...
                current_dns_maps = gateway.run_pyats_command('sh run | in ip host').split('\r\n')
                new_dns_maps = [f'{current_dns_map} {new_validator_ip_list}' for current_dns_map in current_dns_maps]
            ...
            if major_manager_release < 20 or major_iosxe_release < 17:
                exit(f'Unsupported software combination: '
            ...
            if edge_config_type == 1:  # workaround for https://github.com/CiscoDevNet/sastre/issues/12 and .../13
                ...
                for next_num_str in new_nodes_nums:
                    ...
                    variables_payload['devices'].append({'device-id': uuid, 'variables': [
                        {'name': 'system_ip', 'value': f'10.0.0.{next_num_str}'},
                        ...
                        {'name': 'vpn1_gi3_lan_ip', 'value': f'192.168.{next_num_str}.1'},
                        {'name': 'vpn1_gi3_dhcp_network', 'value': f'192.168.{next_num_str}.0'},
                        {'name': 'vpn1_gi3_dhcp_address_exclude', 'value': [f'192.168.{next_num_str}.1']},
                        {'name': 'vpn1_gi3_dhcp_default_gateway', 'value': f'192.168.{next_num_str}.1'},
                        {'name': 'vpn0_gi1_inet_ip', 'value': f'172.16.1.{next_num_str}'},
                        {'name': 'vpn0_gi2_mpls_ip', 'value': f'172.16.2.{next_num_str}'},
            ...
            for next_num_str in new_nodes_nums:
                ...
                track_progress(log, f'Adding nodes to topology ({i}/{len(new_nodes_nums)})...')
                next_node_x_position += 120
                node = lab.create_node(label=f'Edge{next_num_str}', node_definition=cml_node_type,
                                       image_definition=image_definition,
                                       configuration=bootstrap_configs[next_num_str], populate_interfaces=True,
                                       x=next_node_x_position, y=320)
                # Need to wait few seconds as otherwise the next step might fail
                # as interfaces will not yet be added to the new node
                time.sleep(5)
            ...
            major_manager_release = manager_version.major
            minor_manager_release = manager_version.minor
            if major_manager_release < 20 or (major_manager_release == 20 and minor_manager_release < 12):
                sys.exit('SD-WAN Manager versions lower than 20.12 do not support sd-routing.')
            ...
            for next_num_str in new_nodes_nums:
                ...
                track_progress(log, f'Adding nodes to topology ({i}/{len(new_nodes_nums)})...')
                next_node_x_position += 120
                node = lab.create_node(label=f'SD-Edge{next_num_str}', node_definition=cml_node_type,
