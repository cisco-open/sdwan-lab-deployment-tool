...
_CONTROLLER_BOOT_TIMEOUT = 600
_CONTROLLER_BOOT_INTERVAL = 10
    ...
    with Progress(
        ...
        try:
            ...
            for i, (node, ip) in enumerate(zip(nodes, controller_ips), 1):
                ...
                _add_controller_retrying(client, ip, timeout=_CONTROLLER_BOOT_TIMEOUT)
...
def _detect_ip_type(lab: Lab) -> str:
    ctrl = next((n for n in lab.nodes() if _CTRL_NUM_RE.match(n.label)), None)
    if ctrl is None:
        return "v4"
    ...
    while True:
        try:
            ...
            if remaining <= 0:
                ...
                raise typer.Exit(1)
            time.sleep(min(_CONTROLLER_BOOT_INTERVAL, remaining))
